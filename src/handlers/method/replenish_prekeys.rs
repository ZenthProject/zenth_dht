...
use diesel::prelude::*;
use zenth_dto::{ReplenishPreKeysRequest, ReplenishPreKeysResponse};

pub async fn replenish_prekeys(req: ReplenishPreKeysRequest) -> Result<ReplenishPreKeysResponse, String> {
    if req.username_hash.is_empty() || req.username_hash.len() != 32 {
        return Ok(ReplenishPreKeysResponse {
            success: false,
            total_prekeys: 0,
    ...
    // Retourne le total restant (non utilisées)
    let total: i64 = one_time_prekeys::table
    ...
    Ok(ReplenishPreKeysResponse {
        success: true,
        total_prekeys: total as u32,
